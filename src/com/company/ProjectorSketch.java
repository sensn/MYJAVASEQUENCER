...
public class ProjectorSketch extends PApplet{
    ...
    public ProjectorSketch(){

    }

    public void setOtherWindowRef(Mixer refOtherWin){
        themixer = refOtherWin;
    }
    public void runIt(){
    ...
    public void pre() {
        ...
        if (w != width || h != height) {
            ...
            for (int i = 0; i < myHouse.rooms.length; i++) {
                myHouse.rooms[i].buttons[88].setX (bblock * 0.125f,bblock / 2 + bblock * 10.4f,bblock,bblock);
    ...
    public void setup() {
        ...
        this.surface.setTitle("THE SEQUENCER");
        ...
        public void checkbuttons() {
            for (int i = 0; i < numchannels; i++) {
                ...
                if (myHouse.rooms[i].buttons[94].on == true) {
                    myHouse.rooms[mychannel - 1].prg += 1;         //ONLY RECEIVERS ON CHANGE send +1 -1
                    myHouse.rooms[mychannel - 1].bank += 1;
                    shedMidi.sendMidiBank(mychannel, myHouse.rooms[mychannel - 1].bank, ((int) myHouse.rooms[mychannel - 1].prg));   //TODO HERE
                    myHouse.rooms[mychannel - 1].prg -= 1;
                    shedMidi.sendMidiBank(mychannel, myHouse.rooms[mychannel - 1].bank, ((int) myHouse.rooms[mychannel - 1].prg));   //TODO HERE
                    //System.out.println( "94");
                    //System.out.println( "94: Chennel: " + mychannel + " bank:" +myHouse.rooms[mychannel - 1].bank+" Prg:" + myHouse.rooms[mychannel - 1].prg);
                ...
                if (myHouse.rooms[i].buttons[95].on == true) {
                    ...
                    myHouse.rooms[mychannel - 1].prg -= 1;
                    shedMidi.sendMidiBank(mychannel, myHouse.rooms[mychannel - 1].bank, ((int) myHouse.rooms[mychannel - 1].prg));   //TODO HERE
                    // System.out.println( "965: Chennel: " + mychannel + " bank:" +myHouse.rooms[mychannel - 1].bank+" Prg:" + myHouse.rooms[mychannel - 1].prg);
        ...
        public void over() {
            ...
            if (id == 1) {
                ...
                if (m1val != m1val1) {
                    ...
                    //sendMidiAF(m1val,id);
                    m1val1 = m1val;
                    themixer.thechannel=mychannel-1;
                    tempo = myHouse.rooms[mychannel - 1].instances[0].y;
                    //  themixer.cp51.getController("slider").setValue(myHouse.rooms[mychannel - 1].instances[0].y);
                    themixer.cp51.getController("tempoSlider").setBroadcast(false);
                    themixer.cp51.getController("tempoSlider").setValue(m1val);
                    themixer.cp51.getController("tempoSlider").setBroadcast(true);
                }
            }
            if (id == 2) {
